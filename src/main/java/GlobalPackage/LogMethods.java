...
import static GlobalPackage.GeneralConfigs.getLogFolder;
...
public class LogMethods
{
    ...
    private static final String logFolder = GeneralConfigs.getLogFolder();
    private static final String condottaFolder = GeneralConfigs.getReportDump_MessaggiSupervisor();
    private static int counter = 0;
    private static int counterLog = 1;
    private static LocalDate lastLogDate = null;
    ...
    public static void WriteLogCondotta(String message)
    {
        try {
            writeLogToFileCondotta(message);
        } catch (IllegalStateException ignored)
        {
            System.out.println(message);
        } catch (Exception e) {
            System.out.println("LogMethods::WriteLogCondotta->" + e);
        }
    }
    ...
    public static void WriteLogServer(String message)
    {
        ...
        LogSender.sendHttps(LogSender.extractVariables(message,"Server"),"https://scanteq.com/php/supervisor.php");
    ...
    public static void WriteLogDelete(String message)
    {
        ...
        LogSender.sendHttps(LogSender.extractVariables(message,"Delete"),"https://scanteq.com/php/supervisor.php");
    ...
    public static void WriteLogError(String message)
    {
        ...
        LogSender.sendHttps(LogSender.extractVariables(message,"Error"),"https://scanteq.com/php/supervisor.php");
    }
    ...
    public static void WriteLogDebug(String message)
    {
        if (logDebug) {
            ...
            LogSender.sendHttps(LogSender.extractVariables(message,"Debug"),"https://scanteq.com/php/supervisor.php");
        }
    }
    ...
    private static void writeLogToFile(String message) {
        try {
            if (writeToFile) {
                ...
                if (lastLogDate == null || !currentDate.equals(lastLogDate)) {
                    counterLog = 1;
                    lastLogDate = currentDate;
                }
                String s = "\\Supervisor Log " + currentDate + " " + counterLog + ".txt";
                if (!countlines(s)) {
                    FileWriter fw = new FileWriter(logFolder + s, true);
                    fw.write(message + "\r\n");
                    fw.close();
                } else {
                    counterLog++;
    ...
    private static void writeLogToFileCondotta(String message)
    {
        try {
            counter ++;
            if (writeToFile) {
                ...
                FileWriter fw = new FileWriter(condottaFolder + "\\Supervisor Log " + counter + ".txt"); //the true will append the new data
                fw.write(message + "\r\n");//appends the string to the file
                fw.close();
            }
        } catch (FileNotFoundException e) {
            System.out.println("LogMethods::writeLogToFileCondotta->" + e + "\n Probably \"Log folder\" parameter was not found in the config file.");
        } catch (Exception e) {
            System.out.println("LogMethods::writeLogToFileCondotta->" + e);
        }
    }
    ...
    private static int contaRighe(File file) throws IOException {
        int contatore = 0;
