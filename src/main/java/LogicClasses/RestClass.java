...
public class RestClass
{
    ...
    public RestClass(String yourUri) throws Exception
    {
        if (!GeneralConfigs.getSetupTest()) {
            ...
            LogMethods.WriteLogServer("Trying -2");
            MY_CLIENT = HttpClient.newBuilder().sslContext(createSslContext2()).build();
            LogMethods.WriteLogServer("Trying -1");
            ...
            try {
                ...
                LogMethods.WriteLogServer("Trying 3");
                valid = true;
    ...
    private SSLContext createSslContext() throws Exception
    {
        ...
        keyFactory.init(ks, "pass".toCharArray());
    ...
    {
        char[] keystorePassword = "123456".toCharArray(); // Sostituisci con la password del keystore
    ...
    //Send Report
    public HttpResponse<String> send_RetryProgressDeclaration(HttpRequest.BodyPublisher body)
    {
        try {
            var localUri = URI.create(myUri.toString() + path_RetryProgressDeclaration);
            //WriteLog("Sending report to: "+localUri.toString());
